...
export const books: BookRecord[] = rawBooks.map(([slug, name, testament, category, chapterCount], index) => ({
  id: String(index + 1), slug, name, testament, category, chapterCount,
}));
...
export function chapterFor(bookSlug: string, chapter: number) {
  const book = books.find((item) => item.slug === bookSlug);
  if (!book) return null;
  const texts = chapterTexts[`${bookSlug}:${chapter}`] ?? chapterTexts["mark:1"];
  return {
    book,
    chapter,
    verses: texts.map((text, index) => ({
      ...
      verse: index + 1,
      text,
      reference: `${book.name} ${chapter}:${index + 1}`,
      translation: "KJV",
    })),
  };
}
...
].map(([slug, name, role]) => ({ slug, name, role, description: `${name} is connected to key moments in the biblical story. Explore the passages, places, and events associated with this life.` }));
...
].map(([slug, name, region, confidence]) => ({ slug, name, region, confidence }));
...
].map(([slug, name, summary, period]) => ({ slug, name, summary, period }));

export const timeline = events.map((event, index) => ({
  slug: event.slug,
  title: event.name,
  period: event.period,
...
export const journey = {
  ...
  ].map(([title, location, description, scriptureReferences, placeSlug]) => ({ title, location, description, scriptureReferences, placeSlug })),
};

export function verseById(id: string) {
  const [bookSlug, chapterRaw, verseRaw] = id.split(/-(?=\d)/);
  const chapter = chapterFor(bookSlug || "mark", Number(chapterRaw || 1));
  return chapter?.verses.find((item) => item.verse === Number(verseRaw || 16)) ?? chapterFor("mark", 1)!.verses[15];
}
